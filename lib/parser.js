...
function Html2ReactParser(options) {
  ...
  function traverseDom(node, isValidNode, processingInstructions, index, args) {
    args = args || {};
    if (isValidNode(node)) {
      ...
      if (processingInstruction != null) {
        var children = reject(function (x) {return x == null || x === false;},
          addIndex(map)(function (child, i) {
            return traverseDom(child, isValidNode, processingInstructions, i, args);
          }, node.children || []));
        ...
        if (processingInstruction.replaceChildren) {
          ...
          return processingInstruction.processNode(node, children, index, args);
        }
      } else {
        return false;
      }
  ...
  function parseWithInstructions(html, isValidNode, processingInstructions, args) {
    var domTree = parseHtmlToTree(html);

    var list = domTree.map(function (domTreeItem, index) {
      return traverseDom(domTreeItem, isValidNode, processingInstructions, index, args);
    });
